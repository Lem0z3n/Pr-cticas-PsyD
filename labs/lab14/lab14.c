...
void Task8( void *id) // Cada vez que reciba un scancode lo escribe en el LCD
{
	...
	{
		scancode = *((uint8 *) OSMboxPend( keypadMbox, 0, &err ));
		lcd_draw_box( 10, 10, 155, 230, BLACK, 5 );
		lcd_puthex_x2(200, 115, BLACK, scancode );
	}

}

void Task9( void *id) // Escribe cada segundo transcurrido desde el inicio del programa en el LCD
{
    INT8U err;
	static uint32 og_time;
	...
	while(1)
	{
        OSTimeDly( 100 );
		og_time++;
		lcd_draw_box( 165, 10, 310, 230, BLACK, 5 );
		lcd_putint_x2(125, 115, BLACK, og_time );
	}
